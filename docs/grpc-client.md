...
- [src/HalDisplay.cpp](../src/HalDisplay.cpp) — hidden window; copy
  `pixelBuf` on the SDL thread when a snapshot is pending
...
thread owns the channel. `simulator_main`, `HalDisplay::presentIfNeeded`,
and `HalGPIO::update` must not call RPC.
...
`HalDisplay::presentIfNeeded` copies `pixelBuf` (optional region),
